 ...
 * 还有一种终结器引用：终结器引用无需手动编码，但其内部配合引用队列使用，在垃圾回收第一次扫描时，终结器引用入队（被引用对象暂时没有被回收），
 * 再由Finalizer线程通过终结器引用找到被引用对象并调用它的finalize方法，第二次GC扫描时回收被引用对象。
 * <p>
 * 垃圾回收算法：
 ...
 * <p>
 * <p>
 * 方法区用于存储已被虚拟机加载的类信息、常量、静态变量、动态生成的类等数据。实际上在Java虚拟机的规范中方法区是堆中的一个逻辑部分，但是
 * 它却拥有一个叫做非堆(Non-Heap)的别名。对于方法区的实现，不同虚拟机中策略也不同。以我们常用的HotSpot虚拟机为例，其设计团队使用永久
 * 带来实现方法区，并把GC的分代收集扩展至永久带。这样设计的好处就是能够省去专门为方法区编写内存管理的代码。但是在实际的场景中，这样的实现
 * 并不是一个好的方式，因为永久带有MAX上限，所以这样做会更容易遇到内存溢出问题。
 * 关于方法区的GC回收，Java虚拟机规范并没有严格的限制。虚拟机在实现中可以自由选择是否实现垃圾回收。主要原因还是方法区内存回收的效果比较
 * 难以令人满意，方法区的内存回收主要是针对常量池(1.7已经将常量池逐步移出方法区)以及类型的卸载，但是类型卸载在实际场景中的条件相当苛刻。
